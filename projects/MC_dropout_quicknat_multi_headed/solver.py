        ...
        for epoch in range(self.start_epoch, self.num_epochs + 1):
            ...
            for phase in ['train', 'val']:
                ...
                for i_batch, sample_batched in enumerate(dataloaders[phase]):
                    ...
                    ds = sample_batched[2].type(torch.FloatTensor)
                    ...
                    if model.is_cuda:
                        ...
                        ds = ds.cuda(self.device, non_blocking=True)
                    ...
                    intermediate_loss = self.loss_func(intermediate_output, (y, ds), (None, None))
                    ...
                    _, batch_output = torch.max(output[0], dim=1)
                    _, scalar_batch_output = torch.max(output[1], dim=1)
                    out_list.append(batch_output.cpu())
                    scalar_out_list.append(scalar_batch_output.cpu())
                    y_list.append(y.cpu())
                    ds_list.append(ds.cpu())

                    del X, y, intermediate_output, output, batch_output, loss, ce_loss, \
                        dice_loss, kl_div_loss, intermediate_loss, scalar_batch_output, ds
                    torch.cuda.empty_cache()
                ...
                with torch.no_grad():
                    out_arr, scalar_arr, y_arr, ds_arr = torch.cat(out_list), torch.cat(scalar_out_list), torch.cat(y_list), torch.cat(ds_list)
                    ...
                    if phase == 'val':
                        ...
                        self.logWriter.image_per_epoch_multi_headed(model.predict(dataloaders[phase].dataset.X[index], self.device),
                                                       model.predict(dataloaders[phase].dataset.X[index], self.device),
                                                       ([dataloaders[phase].dataset.y[index]], [dataloaders[phase].dataset.ds[index]]),
                                                       dataloaders[phase].dataset.X[index], phase, epoch)
                        # self.logWriter.log(','.join(scalar_arr), phase)
                        # self.logWriter.log(','.join(ds_arr), phase)
                    else:
