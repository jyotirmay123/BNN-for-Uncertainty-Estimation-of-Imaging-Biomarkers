...
spec = importlib.util.find_spec('.data_utils', package='dataset_groups.whole_body_datasets')
...
class Evaluator(m.DataUtils):
    ...
    def print_report(self, strin, final=False):
        if final:
            report_file = os.path.join(self.base_dir, 'reports', self.model_name, self.dataset,
                                       str(self.mc_sample) + '_' + self.ctime + '_final_report.txt')
        else:
            report_file = os.path.join(self.base_dir, 'reports', self.model_name, self.dataset,
                                       str(self.mc_sample) + '_' + self.ctime + '_report.txt')
    ...
    def evaluate_dice_score(self, prediction_path, load_from_txt_file=True, device=0, logWriter=None,
        ...
        with torch.no_grad():
            for vol_idx, file_path in enumerate(file_paths):
                ...
                # labelmap = np.squeeze(labelmap[pickid-1])
                ...
                for i in range(0, len(volume), batch_size):
                    ...
                    if self.is_uncertainity_check_enabled:
                        ...
                        # for mcs, o in enumerate(out):
                        #     o = F.softmax(o, dim=1)
                        #     _, batch_class_map = torch.max(o, dim=1)
                        #     iou_uncertainty[mcs, i: i + batch_size] = batch_class_map.cpu().numpy()
                        #     batch_output_ = (out.cpu().numpy()).astype('float32')
                        #     batch_uncertainty_outputs.append(batch_output_)
                    ...
                    # print('time_taken for 2 slice:', te-ts)

                s_ncc_list.append(self.variance_ncc_dist(iou_uncertainty, labelmap.unsqueeze(dim=0).numpy()))
                s_ged_list.append(
                    self.generalised_energy_distance(iou_uncertainty, labelmap.unsqueeze(dim=0).numpy(), 3))
